const { getPokemonById, checkPokemonRarity, isPseudoLegendary} = require('../pokeService');
    ...
    try{
        ...
        setInterval(async () => {
            ...
            while(true) {
                ...
                pokemonRarity = await checkPokemonRarity(randomPokemonId)
                isPseudo = await isPseudoLegendary(randomPokemonId)
        ...
        }, 20000); // 30000 milissegundos = 30 segundos
...
function removeActivePokemon(channelId, pokemonId) {
    if (activePokemon[channelId]) {
        ...
        // Remover a lista do canal se estiver vazia
...
async function despawnPokemon(channel, pokemonData) {
    ...
    const embed = new Discord.EmbedBuilder()
    .setColor(0xff0000) 
    .setTitle(`${pokemonName.toUpperCase()} fugiu!`)
    .setDescription("Você não foi rápido o suficiente para capturá-lo.")
    .setTimestamp();
